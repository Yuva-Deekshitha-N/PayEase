...
export function MerchantLayout({ user, onLogout }: MerchantLayoutProps) {
  ...
  const navigationItems = [
    ...
    { id: 'users', label: 'User Tracking', icon: Users, path: '/users' },
    { id: 'analytics', label: 'Analytics', icon: BarChart3, path: '/analytics' },
    { id: 'risk', label: 'Risk Dashboard', icon: Shield, path: '/risk' },
    { id: 'profile', label: 'Business Profile', icon: User, path: '/profile' },
  ];

  const isActive = (path: string) => location.pathname === path;

  const handlePaymentConnect = (paymentData: any) => {
    // Update merchant payment information
    console.log('Merchant payment method connected:', paymentData);
  };
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <Badge 
                  variant={user.businessVerified ? 'default' : 'secondary'} 
                  className={`text-xs ${user.businessVerified ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200'}`}
                >
      ...
      <div className="lg:ml-64">
        ...
        <header className="h-16 bg-card/50 backdrop-blur-sm border-b border-border/50 flex items-center justify-between px-4 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <Avatar 
              className="cursor-pointer ring-2 ring-border/50 hover:ring-blue-500/50 transition-all"
